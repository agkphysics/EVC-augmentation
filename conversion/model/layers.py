...
class SpeakerClassifier(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.projection_to_A = LinearNorm(hparams.SC_hidden_dim, len(hparams.emo_list), bias=False)
...
class SpeakerEncoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.lstm = nn.LSTM(hparams.n_mel_channels, int(hparams.emotion_encoder_hidden_dim / 2),
                            num_layers=2, batch_first=True,  bidirectional=True, dropout=hparams.emotion_encoder_dropout)
        self.projection1 = LinearNorm(hparams.emotion_encoder_hidden_dim,
                                      hparams.emotion_embedding_dim,
                                      w_init_gain='tanh')
        self.projection2 = LinearNorm(hparams.emotion_embedding_dim,
...
class AudioEncoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        if hparams.spemb_input:
            input_dim = hparams.n_mel_channels + hparams.emotion_embedding_dim
        else:
...
class AudioSeq2seq(nn.Module):
    ...
    def inference_beam(self, x, start_embedding, embedding_table,
        ...
        # Just in case the best hypothesis is simply the stop token
        i = next((i for i, h in enumerate(hyps) if len(h) > 1), 0)

        return hiddens[i].unsqueeze(0), hyps[i].unsqueeze(0), attn[i].unsqueeze(0)
