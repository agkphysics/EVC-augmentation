...
class ParrotLoss(nn.Module):
    def __init__(self, hparams):
        ...
        self.spenc_w = hparams.emotion_encoder_loss_w
        self.texcl_w = hparams.text_classifier_loss_w
        self.spadv_w = hparams.emotion_adversial_loss_w
        self.spcla_w = hparams.emotion_classifier_loss_w
    ...
    def parse_targets(self, targets, text_lengths):
        ...
        emotion_target [batch_size]
        stop_target [batch_size, T]
        '''
        text_target, mel_target, spc_target, emotion_target, stop_target = targets

        B = stop_target.size(0)
        stop_target = stop_target.reshape(B, -1, self.n_frames_per_step)
        stop_target = stop_target[:, :, 0]

        # padded = torch.tensor(text_target.new(B,1).zero_())
        padded = text_target.new_zeros(B, 1)
        ...
        return text_target, mel_target, spc_target, emotion_target, stop_target
    ...
    def forward(self, model_outputs, targets, eps=1e-5):
        ...
        emotion_logit_from_mel [B, n_emotions]
        emotion_logit_from_mel_hidden [B, max_text_len, n_emotions]
        text_lengths [B,]
        mel_lengths [B,]
        '''
        ...
        text_target, mel_target, spc_target, emotion_target, stop_target  = self.parse_targets(targets, text_lengths)
        ...
        n_emotions = emotion_logit_from_mel_hidden.size(2)
        TTEXT = emotion_logit_from_mel_hidden.size(1)
        n_symbols_plus_one = text_logit_from_mel_hidden.size(2)

        emotion_encoder_loss = torch.tensor(0.).cuda()
        emotion_encoder_acc = torch.tensor(0.).cuda()
        ...
        emotion_logit_flatten = emotion_logit_from_mel_hidden.reshape(-1, n_emotions) # -> [B* TTEXT, n_emotions]
        predicted_emotion = torch.softmax(emotion_logit_flatten, dim=1).argmax(1).long()
        emotion_target_flatten = emotion_target.unsqueeze(1).expand(-1, TTEXT).reshape(-1)
        emotion_classification_acc = ((predicted_emotion == emotion_target_flatten).float() * text_mask.reshape(-1)).sum() / text_mask.sum()
        loss = self.CrossEntropyLoss(emotion_logit_flatten, emotion_target_flatten)
        emotion_classification_loss = torch.sum(loss * text_mask.reshape(-1)) / torch.sum(text_mask)

        # emotion adversival loss #
        flatten_target = torch.empty_like(emotion_logit_flatten).fill_(1. / n_emotions)
        loss = self.MSELoss(torch.softmax(emotion_logit_flatten, dim=1), flatten_target)
        mask = text_mask.unsqueeze(2).expand(-1,-1, n_emotions).reshape(-1, n_emotions)
        emotion_adversial_loss = torch.sum(loss * mask) / torch.sum(mask)

        loss_list = [recon_loss, recon_loss_post,  stop_loss,
                contrast_loss,  emotion_encoder_loss, emotion_classification_loss,
                text_classification_loss, emotion_adversial_loss]

        acc_list = [emotion_encoder_acc, emotion_classification_acc, text_classification_acc]

        combined_loss1 = recon_loss + recon_loss_post + stop_loss + self.contr_w * contrast_loss +  \
            self.texcl_w * text_classification_loss + \
            self.spadv_w * emotion_adversial_loss

        combined_loss2 = self.spcla_w * emotion_classification_loss
