...
from model import Parrot, ParrotLoss
...
def prepare_dataloaders(hparams):
    ...
    trainset = TextMelIDLoader(hparams.training_list, hparams.mel_mean_std,
        hparams.phones_csv, hparams.emo_csv, hparams.emo_list, pids=None, shuffle=True)
    valset = TextMelIDLoader(hparams.validation_list, hparams.mel_mean_std,
        hparams.phones_csv, hparams.emo_csv, hparams.emo_list, pids=None)
    collate_fn = TextMelIDCollate(np.lcm(hparams.n_frames_per_step_encoder,
                                         hparams.n_frames_per_step_decoder))

    train_sampler = DistributedSampler(trainset) if hparams.distributed_run else None

    train_loader = DataLoader(trainset, num_workers=5,
...
def load_model_for_inference(hparams, checkpoint_path):
    ...
    model.load_state_dict(modify_state_dict(checkpoint['state_dict']), strict=False)
    iteration = checkpoint['iteration']
    print(f"Loaded model from checkpoint {checkpoint_path} at iteration {iteration}")
    return model
...
def modify_state_dict(state_dict):
    for key in list(state_dict):
        ...
        #     # Copy encoders
        #     newkey = key.replace("speaker_encoder", "emotion_encoder")
        #     state_dict[newkey] = state_dict[key]
        if key.startswith("speaker_classifier"):
            newkey = key.replace("speaker_classifier", "emotion_classifier")
...
def warm_start_model(checkpoint_path, model):
    ...
    model.load_state_dict(modify_state_dict(checkpoint_dict['state_dict']), strict=False) #
    return model
...
def load_checkpoint(checkpoint_path, model, optimizer_main, optimizer_sc):
    ...
    checkpoint_dict = torch.load(checkpoint_path, map_location='cpu')
    model.load_state_dict(modify_state_dict(checkpoint_dict['state_dict']))
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        val_loader = DataLoader(valset, sampler=val_sampler, num_workers=5,
        ...
        for i, batch in enumerate(train_loader):
            ...
            if rank == 0 and (iteration % hparams.iters_per_checkpoint == 0):
                checkpoint_path = os.path.join(
                    os.path.join(output_directory,log_directory), "checkpoint_{}".format(iteration))
                save_checkpoint(model, optimizer_main, optimizer_sc, learning_rate, iteration,
                                checkpoint_path)
            iteration += 1
    if rank == 0:
        checkpoint_path = os.path.join(os.path.join(output_directory, log_directory), "checkpoint_{}".format(iteration))
        save_checkpoint(model, optimizer_main, optimizer_sc, learning_rate, iteration, checkpoint_path)
...
if __name__ == '__main__':
    ...
    if args.n_gpus > 1 and hparams.distributed_run:
        mp.spawn(train, nprocs=args.n_gpus, args=(args.output_directory, args.log_directory, args.checkpoint_path,
                 args.warm_start, args.n_gpus, args.group_name, hparams))
    else:
        train(args.rank, args.output_directory, args.log_directory, args.checkpoint_path,
              args.warm_start, args.n_gpus, args.group_name, hparams)
